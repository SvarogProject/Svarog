...
[System.Serializable]
public class _2dxFX_Shiny_Reflect : _moiling_2dxFX_BaseClass
{
	...
	{	
		...
		if (ActiveChange)
		{
			...
			if (UseShinyCurve)
			{
				rendererMaterial.SetFloat("_Distortion", ShinyLightCurve.Evaluate(ShinyLightCurveTime));
				ShinyLightCurveTime += (Time.deltaTime/8)*AnimationSpeedReduction;
			}
			else
			{
				rendererMaterial.SetFloat("_Distortion", Light);
			}

			rendererMaterial.SetFloat("_Value2", LightSize);
			rendererMaterial.SetFloat("_Value3", Intensity);
			rendererMaterial.SetFloat("_Value4", OnlyLight);
			rendererMaterial.SetFloat("_Value5", LightBump);
		}
