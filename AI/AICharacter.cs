...
public class AICharacter : MonoBehaviour {
    ...
    float
        blockMultiplier; //we didn't use this in the end, but you can add to the percentage of if it's going to block or take damage
...
/*        float delay = a.attacks[index].delay;
        states.AttackP = a.attacks[index].attack1;
        states.AttackK = a.attacks[index].attack2;
        yield return new WaitForSeconds(delay);

        states.AttackP = false;
        states.AttackK = false;*/

        if (index < a.attacks.Length - 1) {
            index++;
            StartCoroutine(OpenAttack(a, index));
        }
        yield return new WaitForSeconds(1); // TODO 乱加的
    }
